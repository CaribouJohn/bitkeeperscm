...
import { InputBox, Bitkeeper, API, Repository, Remote, RepositoryState, Branch, ForcePushMode, Ref, Submodule, Commit, Change, RepositoryUIState, Status, LogOptions, APIState, CommitOptions, RefType, CredentialsProvider, BranchQuery, PushErrorHandler, PublishEvent, FetchOptions, RemoteSourceProvider, RemoteSourcePublisher, PostCommitCommandsProvider } from './bitkeeper';
import { Event, SourceControlInputBox, Uri, SourceControl, Disposable, commands } from 'vscode';
import { combinedDisposable, mapEvent } from '../util';
import { toBitkeeperUri } from '../uri';
//import { BitkeeperExtensionImpl } from './extension';
//import { BitkeeperBaseApi } from '../git-base';
...
export class ApiRepositoryState implements RepositoryState {
  ...
  readonly onDidChange: Event<void> = this._repository.onDidRunBitkeeperStatus;
...
export class ApiBitkeeper implements Bitkeeper {

  get path(): string { return this._model.bitkeeper.path; }
...
export class ApiImpl implements API {
  ...
  readonly bitkeeper = new ApiBitkeeper(this._model);
  ...
  toBitkeeperUri(uri: Uri, ref: string): Uri {
    return toBitkeeperUri(uri, ref);
  }
  ...
  async init(root: Uri): Promise<Repository | null> {
    ...
    await this._model.bitkeeper.init(path);
  ...
  registerRemoteSourceProvider(provider: RemoteSourceProvider): Disposable {
    ...
    disposables.push(BitkeeperBaseApi.getAPI().registerRemoteSourceProvider(provider));
...
export function registerAPICommands(extension: BitkeeperExtensionImpl): Disposable {
  const disposables: Disposable[] = [];

  disposables.push(commands.registerCommand('bitkeeper.api.getRepositories', () => {
    const api = extension.getAPI(1);
    return api.repositories.map(r => r.rootUri.toString());
  }));

  disposables.push(commands.registerCommand('bitkeeper.api.getRepositoryState', (uri: string) => {
    const api = extension.getAPI(1);
  ...
  disposables.push(commands.registerCommand('bitkeeper.api.getRemoteSources', (opts?: PickRemoteSourceOptions) => {
    return commands.executeCommand('bitkeeper-base.api.getRemoteSources', opts);
  }));
