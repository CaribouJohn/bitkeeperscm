...
import { memoize, sequentialize, debounce } from './decorator';
import { dispose, anyEvent, filterEvent, isDescendant, pathEquals, toDisposable, eventToPromise } from './util';
import { Bitkeeper } from './bitkeeper';
import * as path from 'path';
import * as fs from 'fs';
import * as nls from 'vscode-nls';
import { fromBitkeeperUri } from './uri';
import { APIState as State, CredentialsProvider, PushErrorHandler, PublishEvent, RemoteSourcePublisher, PostCommitCommandsProvider } from './api/bitkeeper';
...
export class Model implements IRemoteSourcePublisherRegistry, IPostCommitCommandsProviderRegistry, IPushErrorHandlerRegistry {
  ...
  private open(repository: Repository): void {
    ...
    const changeListener = repository.onDidChangeRepository((uri: any) => this._onDidChangeRepository.fire({ repository, uri }));
    const originalResourceChangeListener = repository.onDidChangeOriginalResource((uri: any) => this._onDidChangeOriginalResource.fire({ repository, uri }));
    ...
    const checkForSubmodules = () => {
      ...
      repository.submodules
        ...
        .map((r: { path: string; }) => path.join(repository.root, r.path))
        .forEach((p: string) => this.eventuallyScanPossibleGitRepository(p));
    };
  ...
  private getOpenRepository(hint: any): OpenRepository | undefined {
    ...
    if (hint instanceof Uri) {
      ...
      if (hint.scheme === 'git') {
        resourcePath = fromBitkeeperUri(hint).path;
      } else {
