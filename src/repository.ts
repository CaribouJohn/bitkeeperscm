...
import { Branch, Change, ForcePushMode, BitkeeperErrorCodes, LogOptions, Ref, RefType, Remote, Status, CommitOptions, BranchQuery, FetchOptions } from './api/bitkeeper';
//import { AutoFetcher } from './autofetch';
import { debounce, memoize, throttle } from './decorator';
import { Commit, BitkeeperError, Repository as BaseRepository, Submodule, LogFileOptions } from './bitkeeper';
//import { StatusBarCommands } from './statusbar';
import { toBitkeeperUri } from './uri';
...
export interface BitkeeperResourceGroup extends SourceControlResourceGroup {
...
class FileEventLogger {
  ...
  constructor(
    ...
    private onDotBitkeeperFileChange: Event<Uri>,
  ...
  private onDidChangeLogLevel(level: LogLevel): void {
    ...
    this.eventDisposable = combinedDisposable([
      ...
      this.onDotBitkeeperFileChange(uri => this.outputChannelLogger.logDebug(`[.git] Change: ${uri.fsPath}`))
    ]);
  }
...
class DotBitkeeperWatcher implements IFileWatcher {
  ...
  constructor(
    ...
    const rootWatcher = watch(repository.dotBitkeeper.path);
    ...
    repository.onDidRunBitkeeperStatus(this.updateTransientWatchers, this, this.disposables);
  ...
  private updateTransientWatchers() {
    ...
    const upstreamPath = path.join(this.repository.dotBitkeeper.commonPath ?? this.repository.dotBitkeeper.path, 'refs', 'remotes', remote, name);
...
class ResourceCommandResolver {
  ...
  getResources(resource: Resource): [Uri | undefined, Uri | undefined] {
    for (const submodule of this.repository.submodules) {
      if (path.join(this.repository.root, submodule.path) === resource.resourceUri.fsPath) {
        return [undefined, toBitkeeperUri(resource.resourceUri, resource.resourceGroupType === ResourceGroupType.Index ? 'index' : 'wt', { submoduleOf: this.repository.root })];
      }
    }
  ...
  private getLeftResource(resource: Resource): Uri | undefined {
    switch (resource.type) {
      ...
      case Status.INDEX_RENAMED:
      case Status.INDEX_ADDED:
        return toBitkeeperUri(resource.original, 'HEAD');

      case Status.MODIFIED:
      case Status.UNTRACKED:
        return toBitkeeperUri(resource.resourceUri, '~');

      case Status.DELETED_BY_US:
      case Status.DELETED_BY_THEM:
        return toBitkeeperUri(resource.resourceUri, '~1');
    }
    return undefined;
  }
  ...
  private getRightResource(resource: Resource): Uri | undefined {
    switch (resource.type) {
      ...
      case Status.INDEX_DELETED:
      case Status.DELETED:
        return toBitkeeperUri(resource.resourceUri, 'HEAD');

      case Status.DELETED_BY_US:
        return toBitkeeperUri(resource.resourceUri, '~3');

      case Status.DELETED_BY_THEM:
        return toBitkeeperUri(resource.resourceUri, '~2');
...
export class Repository implements Disposable {
  ...
  readonly onDidRunBitkeeperStatus: Event<void> = this._onDidChangeStatus.event;
  ...
  get mergeGroup(): BitkeeperResourceGroup { return this._mergeGroup as BitkeeperResourceGroup; }

  private _indexGroup: SourceControlResourceGroup;
  get indexGroup(): BitkeeperResourceGroup { return this._indexGroup as BitkeeperResourceGroup; }

  private _workingTreeGroup: SourceControlResourceGroup;
  get workingTreeGroup(): BitkeeperResourceGroup { return this._workingTreeGroup as BitkeeperResourceGroup; }

  private _untrackedGroup: SourceControlResourceGroup;
  get untrackedGroup(): BitkeeperResourceGroup { return this._untrackedGroup as BitkeeperResourceGroup; }
  ...
  get dotBitkeeper(): { path: string; commonPath?: string; } {
    return this.repository.dotBitkeeper;
  }
  ...
  constructor(
    ...
    let onRepositoryDotBitkeeperFileChange: Event<Uri>;

    try {
      const dotBitkeeperFileWatcher = new DotBitkeeperWatcher(this, outputChannelLogger);
      onRepositoryDotBitkeeperFileChange = dotBitkeeperFileWatcher.event;
      this.disposables.push(dotBitkeeperFileWatcher);
    } catch (err) {
      outputChannelLogger.logError(`Failed to watch path:'${this.dotBitkeeper.path}' or commonPath:'${this.dotBitkeeper.commonPath}', reverting to legacy API file watched. Some events might be lost.\n${err.stack || err}`);

      onRepositoryDotBitkeeperFileChange = filterEvent(onRepositoryFileChange, uri => /\.git($|\/)/.test(uri.path));
    }

    // FS changes should trigger `git status`:
    // 	- any change inside the repository working tree
    //	- any change whithin the first level of the `.git` folder, except the folder itself and `index.lock`
    const onFileChange = anyEvent(onRepositoryWorkingTreeFileChange, onRepositoryDotBitkeeperFileChange);
    onFileChange(this.onFileChange, this, this.disposables);

    // Relevate repository changes should trigger virtual document change events
    onRepositoryDotBitkeeperFileChange(this._onDidChangeRepository.fire, this._onDidChangeRepository, this.disposables);

    this.disposables.push(new FileEventLogger(onRepositoryWorkingTreeFileChange, onRepositoryDotBitkeeperFileChange, outputChannelLogger));

    const root = Uri.file(repository.root);
    this._sourceControl = scm.createSourceControl('git', 'Bitkeeper', root);
    ...
    this.disposables.push(this.onDidRunBitkeeperStatus(() => this.updateInputBoxPlaceholder()));
...
//        this.disposables.push(new AutoFetcher(this, globalState));
  ...
  provideOriginalResource(uri: Uri): Uri | undefined {
    ...
    return toBitkeeperUri(uri, '', { replaceFileExtension: true });
  }
    ...
    if (this.rebaseCommit) {
      ...
      await this.run(Operation.Commit, async () => {
        ...
        if (opts.requireUserConfig === undefined || opts.requireUserConfig === null) {
          ...
          opts.requireUserConfig = config.get<boolean>('requireBitkeeperUserConfig');
        }
  ...
  async show(ref: string, filePath: string): Promise<string> {
    return await this.run(Operation.Show, async () => {
      ...
      try {
        ...
        if (err.gitErrorCode === BitkeeperErrorCodes.WrongCase) {
          const gitRelativePath = await this.repository.getBitkeeperRelativePath(ref, path);
        ...
        const onExit = (exitCode: number) => {
          if (exitCode === 1) {
            ...
            if (/ is in submodule /.test(stderr)) {
              reject(new BitkeeperError({ stdout: data, stderr, exitCode, bkErrorCode: BitkeeperErrorCodes.IsInSubmodule }));
            } else {
              reject(new BitkeeperError({ stdout: data, stderr, exitCode }));
            }
          }
        };
  ...
  private async run<T>(operation: Operation, runOperation: () => Promise<T> = () => Promise.resolve<any>(null)): Promise<T> {
    ...
    try {
      ...
      if (err.gitErrorCode === BitkeeperErrorCodes.NotABitkeeperRepository) {
    ...
    while (true) {
      try {
        ...
        const shouldRetry = attempt <= 10 && (
          (err.gitErrorCode === BitkeeperErrorCodes.RepositoryIsLocked)
          || ((operation === Operation.Pull || operation === Operation.Sync || operation === Operation.Fetch) && (err.gitErrorCode === BitkeeperErrorCodes.CantLockRef || err.gitErrorCode === BitkeeperErrorCodes.CantRebaseMultipleBranches))
        );
  ...
  private async updateModelState(): Promise<void> {
    ...
    if (didHitLimit && !shouldIgnore && !this.didWarnAboutLimit) {
      ...
      const gitWarn = localize('huge', "The git repository at '{0}' has too many active changes, only a subset of Bitkeeper features will be enabled.", this.repository.root);
